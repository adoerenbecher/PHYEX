...
!
LLINIT=.TRUE.
IF (PRESENT(LDINIT)) THEN
  LLINIT=LDINIT
  WRITE(KLUOUT,'(" INI_PHYEX: LLINIT (GIVEN) = ",L2)') LLINIT
ENDIF
LLCHECK=.TRUE.
IF(PRESENT(LDCHECK)) LLCHECK=LDCHECK
LLCHANGEMODEL=.TRUE.
IF (PRESENT(LDCHANGEMODEL)) THEN
  LLCHANGEMODEL=LDCHANGEMODEL
  WRITE(KLUOUT,'(" INI_PHYEX: LLCHANGEMODEL (GIVEN) = ",L2)') LLCHANGEMODEL
ENDIF
IPRINT=0
...
IF(CMICRO=='ICE3' .OR. CMICRO=='ICE4' .OR. CMICRO=='LIMA') THEN
  ...
  IF(LLINIT) THEN
    CALL INI_RAIN_ICE(HPROGRAM, KLUOUT, PTSTEP, PDZMIN, CLOUDPARN%NSPLITR, CMICRO)
    CALL INI_TIWMX
